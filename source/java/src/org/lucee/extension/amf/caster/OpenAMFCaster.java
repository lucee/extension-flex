...
public final class OpenAMFCaster implements AMFCaster {
	...
	public Object toAMFObject(Object o) throws PageException {
    	...
    	lucee.runtime.util.Decision util = CFMLEngineFactory.getInstance().getDecisionUtil();
    	Cast caster = CFMLEngineFactory.getInstance().getCastUtil();
        if(util.isBinary(o))    return o;
        if(util.isArray(o))     return toAMFObject(caster.toList(o,true));
        if(util.isBoolean(o))   return caster.toBoolean(o);
        if(util.isComponent(o)) return toAMFObject((Component)o);
        if(util.isDate(o,false))return caster.toDate(o,null);
        if(util.isNumber(o))   return caster.toDouble(o);
        if(util.isQuery(o))     return toAMFObject(caster.toQuery(o));
        if(util.isStruct(o))    return toAMFObject(caster.toMap(o,true));
        if(util.isUserDefinedFunction(o))
    ...
    private static Object toAMFObject(UDF udf) throws PageException {
        throw CFMLEngineFactory.getInstance().getExceptionUtil()
        		.createApplicationException("can't send a User Defined Function ("+udf.getFunctionName()+") via flash remoting");
    }
    ...
    private Query toCFMLObject(ASRecordSet rs) throws PageException {
        ...
        Query qry=CFMLEngineFactory.getInstance().getCreationUtil().createQuery(columns,len,"query");
